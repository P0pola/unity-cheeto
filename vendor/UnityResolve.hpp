    ...
    struct Method final {
      ...
      public:
        ...
        auto Invoke(Args... args) -> Return {
            Compile();
            if (function)
                return reinterpret_cast<Return(UNITY_CALLING_CONVENTION *)(Args...)>(function)(args...);
            return Return();
        }
        ...
        auto RuntimeInvoke(Obj *obj, Args... args) -> Return {
            ...
            if (mode_ == Mode::Il2Cpp) {
                if constexpr (std::is_void_v<Return>) {
                    ...
                    return Unbox<Return>(Invoke<void *>("il2cpp_runtime_invoke", address, obj, sizeof...(Args) ? argArray : nullptr, nullptr));
            }
            ...
            if constexpr (std::is_void_v<Return>) {
                ...
                return Unbox<Return>(Invoke<void *>("mono_runtime_invoke", address, obj, sizeof...(Args) ? argArray : nullptr, nullptr));
        }
  ...
  private:
    static auto ForeachAssembly() -> void {
        ...
        if (mode_ == Mode::Il2Cpp) {
            ...
            Invoke<void *, void (*)(void *ptr, std::vector<Assembly *> &), std::vector<Assembly *> &>(
        ...
        if (mode_ == Mode::Il2Cpp) {
            ...
            do {
                if ((field = Invoke<void *>("il2cpp_class_get_fields", pKlass, &iter))) {
                    const auto pField = new Field{.address = field,
            ...
            do {
                if ((method = Invoke<void *>("il2cpp_class_get_methods", pKlass, &iter))) {
                    ...
                    pMethod->return_type = new Type{
                        .address = Invoke<void *>("il2cpp_method_get_return_type", method),
                    };
  ...
  public:
    class UnityType final {
      public:
        ...
        enum class MemberTypes : uint32_t { Constructor = 1, Event = 2, Field = 4, Method = 8, Property = 16, TypeInfo = 32, Custom = 64, NestedType = 128, All = 191 };
        ...
        struct CsType {
            ...
            auto GetField(const std::string &name, const BindingFlags flags = static_cast<BindingFlags>(static_cast<int>(BindingFlags::Instance) | static_cast<int>(BindingFlags::Static) |
        ...
        struct Array : Object {
            ...
            auto ToVector() -> std::vector<T> {
                ...
                for (auto i = 0; i < this->max_length; i++)
        ...
        struct GameObject : UnityObject {
            ...
            template <typename T>
            auto GetComponents(Class *type, bool useSearchTypeAsArrayReturnType = false, bool recursive = false, bool includeInactive = true, bool reverse = false, List<T> *resultList = nullptr)
                -> std::vector<T> {
                static Method *method;
                if (!method)
